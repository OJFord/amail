...
<script>
  ...
  $: sortedEvents = parseVCalEvents(vcal).sort((a, b) =>
    a.summary == primaryEventSummary
      ? -1
  ...
  const dateFormat = (date) => date.toJSDate().toLocaleDateString();
